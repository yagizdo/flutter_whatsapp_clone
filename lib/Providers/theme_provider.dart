...
final darkTheme = ThemeData(
  ...
  // Appbar
  appBarTheme: AppBarTheme(color: HexColor('#1B262C'),),
  // Elevated Button
...
final lightTheme = ThemeData(
  ...
  appBarTheme: const AppBarTheme(color: Colors.white,titleTextStyle: TextStyle(color: Colors.black,fontSize: 16)),
...
ThemeProvider() {
  readData('themeMode').then((value) {
    print('value read from lc : ${value.toString()}');
    var themeMode = value ?? 'light';
    if (themeMode == 'light') {
      _themeData = lightTheme;
    } else {
      print('setting dart theme');
